...
// rfc2136_yumc provider type
type rfc2136_yumcProvider struct {
	nameserver    string
	zoneName      string
	tsigKeyName   string
	tsigSecret    string
	...
	dryRun       bool
	actions      rfc2136_yumcActions
}



type rfc2136_yumcActions interface {
	SendMessage(msg *dns.Msg) error
	IncomeTransfer(m *dns.Msg, a string) (env chan *dns.Envelope, err error)
}

// NewRfc2136_yumcProvider is a factory function for OpenStack rfc2136_yumc providers
func NewRfc2136_yumcProvider(host string, port int, zoneName string, insecure bool, keyName string, secret string, secretAlg string, axfr bool, domainFilter endpoint.DomainFilter, dryRun bool, minTTL time.Duration, actions rfc2136_yumcActions) (Provider, error) {
	secretAlgChecked, ok := tsigAlgs[secretAlg]
	if !ok && !insecure {
		return nil, errors.Errorf("%s is not supported TSIG algorithm", secretAlg)
	}

	r := &rfc2136_yumcProvider{
	...
	log.Infof("Configured RFC2136_yumc with zone '%s' and nameserver '%s'", r.zoneName, r.nameserver)
	return r, nil
}

// Records returns the list of records.
func (r rfc2136_yumcProvider) Records(ctx context.Context) ([]*endpoint.Endpoint, error) {
	rrs, err := r.List()
	if err != nil {
		return nil, err
	}
...
func (r rfc2136_yumcProvider) IncomeTransfer(m *dns.Msg, a string) (env chan *dns.Envelope, err error) {
	t := new(dns.Transfer)
	if !r.insecure {
...
func (r rfc2136_yumcProvider) List() ([]dns.RR, error) {
	if !r.axfr {
...
func (r rfc2136_yumcProvider) ApplyChanges(ctx context.Context, changes *plan.Changes) error {
	...
	if len(m.Ns) > 0 {
		...
		if err != nil {
			return fmt.Errorf("RFC2136_yumc update failed: %v", err)
		}
	}
...
func (r rfc2136_yumcProvider) UpdateRecord(m *dns.Msg, ep *endpoint.Endpoint) error {
...
func (r rfc2136_yumcProvider) AddRecord(m *dns.Msg, ep *endpoint.Endpoint) error {
...
func (r rfc2136_yumcProvider) RemoveRecord(m *dns.Msg, ep *endpoint.Endpoint) error {
	...
	for _, target := range ep.Targets {
		...
		log.Debugf("====== RemoveRecord.ep=%#v", ep)
		m.Remove([]dns.RR{rr})
	}
...
func (r rfc2136_yumcProvider) SendMessage(msg *dns.Msg) error {
	if r.dryRun {
